...
/**
 * Simple settings modal (non-portal for simplicity)
 * props:
 *  - open (bool)
 *  - onClose()
 *  - settings (object)
      ...
      <div className="relative z-70 w-full max-w-lg p-6 bg-white dark:bg-gray-800 rounded-lg shadow-xl">
        ...
        <label className="block mb-3 text-sm text-gray-700 dark:text-gray-300">
          ...
          <select
            ...
            onChange={(e) =>
              setLocal({ ...local, autoRefreshInterval: parseInt(e.target.value, 10) })
            }
        ...
        <label className="flex items-center gap-3 mb-3">
          <input
            ...
            onChange={(e) =>
              setLocal({ ...local, reducedMotionPreference: e.target.checked })
            }
            className="rounded border-gray-300 dark:border-gray-600"
          />
          <span className="text-sm text-gray-700 dark:text-gray-300">
            Reduced motion (disable extra animations)
          </span>
        </label>
